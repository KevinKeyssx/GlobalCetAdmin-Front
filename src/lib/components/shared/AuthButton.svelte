		...
		{#snippet trigger( { props } )}
			<button
				...
				class="
					...
					bg-linear-to-br from-brand to-brand-bright
					text-white text-xs font-bold uppercase tracking-wider
					shadow-md hover:shadow-lg hover:scale-105
					transition-all duration-200 cursor-pointer
					ring-2 ring-brand/30 hover:ring-brand/60
					select-none
				"
			...
			<div class="flex flex-col gap-4 min-w-[200px]">
				...
				<div class="flex items-center gap-3">
					...
					<div class="flex flex-col gap-0.5 min-w-0">
						{#if user.name}
							<span class="text-xs font-bold text-text truncate">
								{ user.name }
							</span>
						{/if}
                ...
                <!-- Divider -->
				<div class="h-px bg-brand/10 -mx-1"></div>

				{#if ( darkMode !== undefined && onToggle !== undefined ) }
                    <div class="flex sm:hidden">
                        <ToggleTheme { darkMode } { onToggle } variant="popover" />
                        <!-- Divider -->
                        <div class="h-px bg-brand/10 -mx-1"></div>
                    </div>
				{/if }
