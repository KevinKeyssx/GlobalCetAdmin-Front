...
<div class="space-y-4">
	...
	<div
		...
		class="
			...
			border-2 border-dashed px-6 py-10 text-center transition-all duration-300
			{ isDragging 
	...
	{#if ( files.length > 0 ) }
		<div class="space-y-3">
			<div class="flex items-center justify-between">
				<h4 class="font-display text-xs font-black uppercase tracking-wider text-text-muted">
			...
			<div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
				{#each files as item ( item.id ) }
					<div class="
						flex flex-col gap-3 rounded-xl border border-brand/10 bg-card p-3
						shadow-card transition-all duration-300 hover:border-brand/20
					">
						...
						<div class="grid grid-cols-3 gap-2 border-t border-brand/5 pt-2.5">
							<div class="col-span-2">
								...
								<input
									...
									class="
										w-full rounded-lg border border-brand/15 bg-input px-2.5 py-1 text-xs text-text
										outline-none transition-colors duration-200 focus:border-brand focus:bg-card
									"
								/>
							</div>
