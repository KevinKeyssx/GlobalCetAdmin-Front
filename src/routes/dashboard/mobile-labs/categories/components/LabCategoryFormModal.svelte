<script lang="ts">
	...
	interface LabCategoryFormProps {
		...
		isEditing   : boolean;
		editingId   : string;
		initialData : LabCategoryInitial | null;
		onSave      : () => void;
		onCancel    : () => void;
	}

	interface LabCategoryInitial {
		name : string;
	}
	...
	async function handleSubmit( e : Event ) : Promise<void> {
		...
		try {
			const endpoint = isEditing ? `labs/categories?id=${ editingId }` : 'labs/categories';
...
<DashboardModal
	{ show }
	title={ isEditing ? 'Modificar Categoría' : 'Nueva Categoría de Laboratorio' }
	onClose={ onCancel }
	maxWidth="max-w-md"
>
	{#snippet body()}
