<script lang="ts">
	...
	const categoriesQuery = createQuery( ( ) => ( {
		queryKey : [ 'lab-categories', page, size, debouncedSearch, activeStatus, order, typeOrder ],
		queryFn  : async ( ) : Promise< PaginatedResponse< LabCategory > > => {
			...
			if ( isApiError( response ) ) {
				throw new Error( 'No se pudieron cargar las categorías de laboratorios.' );
			}
