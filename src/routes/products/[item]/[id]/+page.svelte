...
<main class="mx-auto max-w-7xl px-6 py-10 sm:py-16">
	...
	<nav class="mb-8 flex items-center gap-2 font-semibold text-text-muted">
	...
	{:else if isError || !item }
		...
		<div class="flex flex-col items-center justify-center gap-4 py-24 text-center">
			...
			<a
				href  = "/catalog"
				class="mt-4 rounded-xl bg-brand/10 border border-brand/20 px-6 py-3 font-black uppercase tracking-wider text-brand hover:bg-brand hover:text-surface-dark transition-all duration-300"
			>
				Volver al Catálogo
			</a>
		</div>
	...
	{:else}
		...
		<div class="grid grid-cols-1 gap-10 lg:grid-cols-12 lg:gap-16">
				...
				<div class="relative h-[ 360px ] sm:h-[ 480px ] overflow-hidden rounded-3xl border border-brand/10 bg-surface shadow-card group">
					...
					{#if images.length > 1 }
						<button
							...
							class="absolute left-4 top-1/2 -translate-y-1/2 z-20 flex h-10 w-10 items-center justify-center rounded-full bg-black/35 backdrop-blur-md text-white border border-white/10 opacity-0 group-hover:opacity-100 transition-all duration-300 hover:bg-black/60 hover:scale-105 cursor-pointer"
						>
						...
						<button
							...
							class="absolute right-4 top-1/2 -translate-y-1/2 z-20 flex h-10 w-10 items-center justify-center rounded-full bg-black/35 backdrop-blur-md text-white border border-white/10 opacity-0 group-hover:opacity-100 transition-all duration-300 hover:bg-black/60 hover:scale-105 cursor-pointer"
						>
							<svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3">
					...
					<div class="flex items-center justify-center gap-2 pt-2">
						{#each images as _, idx ( idx ) }
							<button
								...
								class="h-2 rounded-full transition-all duration-300 cursor-pointer { idx === activeIndex ? 'bg-brand w-6 shadow-[0_0_8px_var(--color-brand)]' : 'bg-text-muted/20 hover:bg-text-muted/40 w-2' }"
							></button>
						{/each}
					</div>
				{/if}
			...
			<section class="lg:col-span-6 flex flex-col gap-6">
				...
				<!-- Meta Type Badge -->
				<div>
					<span class="rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-widest backdrop-blur-md { itemType === 'lab' ? 'border border-blue-400/30 bg-blue-400/20 text-blue-300' : itemType === 'kit' ? 'border border-emerald-400/30 bg-emerald-400/20 text-emerald-300' : 'border border-brand/30 bg-brand/20 text-brand-bright' }">
				...
				<div class="rounded-2xl border border-brand/10 bg-surface/50 p-6 shadow-card space-y-3">
					...
					{#if shouldTruncate }
						<button
							...
							class="font-bold text-brand hover:text-brand-bright transition-colors duration-200 focus:outline-hidden cursor-pointer"
						>
